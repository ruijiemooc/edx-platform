...
class StubEdxNotesServiceHandler(StubHttpRequestHandler):
    ...
    def _get_paginated_response(self, notes, page_num, page_size):
        ...
        next_url = 'null' if end >= total_notes else _get_next_prev_url(
            url_path, deepcopy(self.get_params), page_num + 1, page_size
        )
        prev_url = 'null' if page_num == 1 else _get_next_prev_url(
            url_path, deepcopy(self.get_params), page_num - 1, page_size)
