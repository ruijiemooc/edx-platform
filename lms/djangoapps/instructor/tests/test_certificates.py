...
class CertificateInvalidationViewTests(SharedModuleStoreTestCase):
    ...
    def setUp(self):
        ...
        self.certificate_invalidation_data = dict(
            user_name=self.enrolled_user_1.username,
            notes=self.notes,
            user_email=self.enrolled_user_1.email,
            user_id=self.enrolled_user_1.id,  # pylint: disable=no-member
        )
    ...
    def test_invalidate_certificate(self):
        ...
        # Assert Certificate Exception Updated data
        self.assertEqual(result['user_email'], self.enrolled_user_1.email)
        self.assertEqual(result['user_name'], self.enrolled_user_1.username)
        ...
        try:
            ...
            self.fail("Certificate not invalidated.")
    ...
    def test_missing_username_and_email_error(self):
        ...
        """
        self.certificate_invalidation_data.update({'user_name': '', 'user_email': ''})
        response = self.client.post(
            self.url,
            data=json.dumps(self.certificate_invalidation_data),
            content_type='application/json',
    ...
    def test_invalid_user_name_error(self):
        ...
        self.certificate_invalidation_data.update({"user_name": invalid_user, "user_email": ""})
    ...
    def test_user_not_enrolled_error(self):
        ...
        """
        self.certificate_invalidation_data.update({"user_name": self.not_enrolled_student.username, "user_email": ""})
    ...
    def test_no_generated_certificate_error(self):
        ...
        """
        self.certificate_invalidation_data.update({"user_name": self.enrolled_user_2.username, "user_email": ""})
