...
It is also helpful to be able to run tests without running all Django migrations (which can be slow) during test
development. To make test runs faster during test development by creating database tables directly from model
definitions you can use the --without_migrations flag to skip all migrations when setting up the test database.
For example to run a single test, without PIP installs, UX builds, or database migrations,
